...
function statusBadge(status: "active" | "placeholder" | "future" | undefined): string {
  if (status === "future") return "Roadmap";
  if (status === "placeholder") return "Setup";
  return "Live";
}

export function ExecutiveCommandCenterLegacy({ baseHref = "/app" }: { baseHref?: string }) {
  ...
  const handlePlaneMove = useCallback((event: MouseEvent<HTMLDivElement>) => {
    ...
    setTilt({ x: (0.5 - offsetY) * 8, y: (offsetX - 0.5) * 8 });
  }, []);
  ...
  return (
    <div className="space-y-4 rounded-[28px] border border-slate-200 bg-[radial-gradient(circle_at_10%_-10%,#dbeafe,transparent_42%),radial-gradient(circle_at_92%_0%,#e0f2fe,transparent_38%),linear-gradient(180deg,#f8fbff_0%,#eff6ff_42%,#f8fafc_100%)] p-4 shadow-[0_20px_60px_rgba(15,23,42,0.08)] lg:p-5">
        ...
        {metrics.map((metric) => (
          ...
          >
            ...
            <div className="mt-2 flex items-center justify-between">
              ...
              <metric.icon className="h-4 w-4 text-blue-500" aria-hidden="true" />
            </div>
      ...
      <section className="grid gap-4 xl:grid-cols-[240px_1fr_300px]">
          ...
          <nav className="space-y-1 overflow-auto pr-1" aria-label="Workspace sections">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="group flex items-center justify-between rounded-xl border border-transparent px-3 py-2 text-sm text-slate-600 transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600"
              >
        ...
        {/* Spatial intelligence plane */}
        <div className="relative overflow-hidden rounded-3xl border border-white/70 bg-white/70 p-3 backdrop-blur">
          <div className="pointer-events-none absolute inset-x-0 -top-10 h-28 bg-[radial-gradient(circle,#bfdbfe_0%,rgba(191,219,254,0)_70%)]" aria-hidden="true" />
          <div className="mb-3 flex items-center justify-between rounded-2xl border border-slate-200 bg-white/80 px-3 py-2">
            ...
            <span className="inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700">
              <CircleDashed className="h-3 w-3" aria-hidden="true" /> WebXR-style topology
            </span>
          </div>
          ...
          >
            ...
            <svg className="pointer-events-none absolute inset-0 h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
              {cityNodes.map((node) => (
                <motion.line
                  key={`line-${node.key}`}
                  x1="50" y1="54" x2={node.x} y2={node.y}
            ...
            {cityNodes.map((city) => {
              ...
              return (
                <motion.button
                  ...
                  aria-label={`Activate ${city.name} department node`}
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Dot className={`h-5 w-5 ${isActive ? "text-blue-500" : "text-slate-300"}`} aria-hidden="true" />
                  </div>
                  ...
                  <div className={`mt-2 h-1.5 rounded-full bg-gradient-to-r ${palette[city.tone]}`} aria-hidden="true" />
                </motion.button>
              );
            })}
          </motion.div>
        ...
        <aside className="space-y-3 rounded-3xl border border-white/70 bg-white/70 p-3 backdrop-blur">
          <div className="rounded-2xl border border-slate-200 bg-white/90 p-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Selected Department</p>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white/90 p-3">
            ...
            <ul className="mt-2 space-y-2 text-xs text-slate-600" aria-label="Churn risk alerts">
              <li className="flex items-center justify-between rounded-xl bg-rose-50 px-2 py-2"><span>BetaTech Industries</span><span className="font-semibold text-rose-700">Score 92</span></li>
              <li className="flex items-center justify-between rounded-xl bg-amber-50 px-2 py-2"><span>Coastal Systems</span><span className="font-semibold text-amber-700">Score 87</span></li>
              <li className="flex items-center justify-between rounded-xl bg-amber-50 px-2 py-2"><span>Bluewave Corp</span><span className="font-semibold text-amber-700">Score 62</span></li>
            </ul>
          </div>

          <div className="rounded-2xl border border-slate-200 bg-white/90 p-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">AI activity feed</p>
            <ul className="mt-2 space-y-2 text-xs text-slate-600" aria-label="AI activity feed" aria-live="polite">
              <li className="flex items-start gap-2"><Bell className="mt-0.5 h-3.5 w-3.5 text-blue-500" aria-hidden="true" /><span>Reply drafted for Global Marine Inc. <strong className="text-slate-700">2m ago</strong></span></li>
              <li className="flex items-start gap-2"><Bell className="mt-0.5 h-3.5 w-3.5 text-blue-500" aria-hidden="true" /><span>Deal stage updated for Skyline Aviation. <strong className="text-slate-700">8m ago</strong></span></li>
              <li className="flex items-start gap-2"><Bell className="mt-0.5 h-3.5 w-3.5 text-blue-500" aria-hidden="true" /><span>Workflow automation completed: onboarding. <strong className="text-slate-700">15m ago</strong></span></li>
            </ul>
          </div>
        </aside>
      </section>
      ...
      <section className="rounded-3xl border border-white/70 bg-white/70 p-3 backdrop-blur">
        <div className="mb-3 flex items-center justify-between rounded-2xl border border-slate-200 bg-white/90 px-3 py-2">
          ...
          <span className="inline-flex items-center gap-1 rounded-full border border-cyan-200 bg-cyan-50 px-3 py-1 text-xs font-semibold text-cyan-700">
            <Cpu className="h-3.5 w-3.5" aria-hidden="true" /> Live orchestration
          </span>
        </div>
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {recommendationCards.map((item) => (
            ...
            >
              ...
              <button
                type="button"
                className="mt-3 inline-flex min-h-[44px] items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700 hover:bg-blue-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600"
              >
                {item.cta}
