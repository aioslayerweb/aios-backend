...
export function AIOSFeatureGrid({ items, columns = 3 }: { items: AIOSFeatureItem[]; columns?: 2 | 3 | 4 }) {
  return (
    <motion.div variants={aiosMotion.staggerChildren} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.15 }}>
      <AIOSGrid columns={columns}>
        {items.map((item) => {
          const Icon = item.icon
          return (
            <motion.div key={item.title} variants={aiosMotion.fadeUp}>
