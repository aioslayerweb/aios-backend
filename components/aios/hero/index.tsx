    ...
    <AIOSSection className="pb-10 pt-20 lg:pt-24">
      <AIOSContainer size="wide">
        <div className="grid items-center gap-12 lg:grid-cols-[1.02fr_0.98fr]">
          <motion.div variants={aiosMotion.fadeUp} initial="hidden" animate="show">
            ...
            {stats?.length ? (
              <div className="mt-12 grid gap-4 sm:grid-cols-3">
                {stats.map((item) => (
                  <div key={item.label} className="public-card public-card-kpi public-card-hover">
          ...
          <motion.div variants={aiosMotion.blurReveal} initial="hidden" animate="show" className="relative">
            {visual}
          </motion.div>
        </div>
      </AIOSContainer>
...
export function AIOSHeroCentered({ eyebrow, title, body, actions }: { eyebrow: string; title: string; body: string; actions?: ReactNode }) {
  return (
    <AIOSSection className="pb-10 pt-20 lg:pt-24">
      <AIOSContainer>
        <motion.div variants={aiosMotion.fadeUp} initial="hidden" animate="show" className="mx-auto max-w-4xl text-center">
