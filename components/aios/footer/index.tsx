            ...
            {includeNewsletter ? (
              <div className="public-card public-card-glass p-4 sm:p-5">
                ...
                <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                  <input value={email} onChange={(event) => setEmail(event.target.value)} type="email" placeholder="Work email" className="public-input" aria-label="Work email" />
              ...
              <div key={group.title}>
                ...
                <div className="mt-4 grid gap-3">
                  {group.links.map((link) => (
                    <Link key={link.href} href={link.href} className="public-small text-[color:var(--public-color-text)] transition-colors hover:text-[color:var(--public-color-primary)]">{link.label}</Link>
                  ))}
                </div>
              </div>
            ))}
